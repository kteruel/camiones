...
Ventcamp = {
    ...
    formInit: function () {
        ...
        doneHandler = function (msg, form) {
            var self = this.Ventcamp;
            if( msg === 'ok' ) {
                form.reset();

                if ( typeof toastr != 'undefined' ) toastr.success(self.defaults.ajaxedFormSuccessMsg);
                else alert(self.defaults.ajaxedFormSuccessMsg);
    ...
    initGoogleMap: function() {
        ...
        if ( $('.map').length ) {
            $('.map').each(function () {
                ...
                var icon = '/bundles/core/frontend/assets/img/marker-46x46.png',
                    address,
                    markerLatLng,
                    offsetX,
                    offsetY,
